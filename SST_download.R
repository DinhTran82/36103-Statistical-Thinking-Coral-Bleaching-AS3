...
NOAA_sub_dl <- function(time_start, time_end, xlon, ylat) {
  NOAA_dat <- griddap(
    ...
    latitude = c(ylat-0.01, ylat+0.01),
    longitude = c(xlon-0.01, xlon+0.01 ),
    fields = c(
    ...
    na.omit() %>% group_by(t) %>%summarise_at(vars("SST","SSTA"), mean)
}

start_date <- as.Date(c("2003-01-01"))
 end_date<- as.Date(c("2020-12-31"))

 
 location <-read_csv("Loc_2006_SST.csv")
...
location
 ...
 system.time(for (i in 1:desired_length) {
   ...
   filename <- sprintf("C:\\Users\\Dinh\\Dropbox\\UTS\\LearnR\\AS3\\datadownload\\data__%003i__%s_%f_%f.csv", i ,location$Reef.Name[i], location$Longitude_coarse[i], location$Latitude_coarse[i] )
...
location$SSTdata[1] 
write_csv(location, "Location_2006_SST.csv")
